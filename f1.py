 ...
 #  start = time.time()
    ...
    '''try:
        for pic, contour in enumerate(sorted_contours):
            xg, yg, w, h = cv2.boundingRect(contour)

            print("Coordnates of green {} , {}".format(xg, yg))

            imageFrame = cv2.rectangle(imageFrame, (xg, yg),
                                       (xg + w, yg + h),
                                       (0, 255, 0), 2)

            cv2.putText(imageFrame, "Green Colour", (xg, yg),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        1.0, (0, 255, 0))
